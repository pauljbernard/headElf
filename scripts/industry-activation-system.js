...
export class IndustryActivationSystem extends EventEmitter {
     ...
     */
    async activateIndustry(industry, config) {
     ...
     */
    async deactivateIndustry(industry) {
     ...
     */
    async configureIndustry(industry, config) {
     ...
     */
    async suggestIndustries(context) {
        const suggestions = [];
        ...
        try {
            ...
            if (config.industryConfigurations) {
                for (const [industry, config] of Object.entries(config.industryConfigurations)) {
                    this.industryConfigurations.set(industry, config);
                }
            }
        }
        catch (error) {
    ...
    async discoverIndustryExtensions() {
        ...
        for (const file of industryFiles) {
            try {
                ...
                const ExtensionClass = Object.values(module).find((value) => value.prototype && value.prototype.industry);
                if (ExtensionClass) {
    ...
    async loadIndustryExtension(industry) {
    ...
    getDefaultConfiguration(industry) {
        return {
            priority: 'MEDIUM',
            autoActivate: false,
    ...
    async calculateIndustryRelevance(industry, extension, context) {
    ...
    async generateSuggestionRationale(industry, context) {
        return `Industry ${industry} shows high relevance based on context analysis`;
    }
    async estimateIndustryBenefit(industry, context) {
        const extension = this.industryExtensions.get(industry);
        return extension ? extension.gdpPercentage / 100 : 0;
    }
    async calculateGdpCoverage() {
        let totalGdp = 0;
        for (const industry of this.activeIndustries) {
            const extension = this.industryExtensions.get(industry);
            if (extension) {
    ...
    async getLastActivationTime(industry) {
        ...
        try {
            ...
            switch (command) {
                case 'activate':
                    ...
                    await activationSystem.activateIndustry(args[0]);
                    break;
                case 'deactivate':
                    if (args.length === 0) {
                    ...
                    await activationSystem.deactivateIndustry(args[0]);
                    break;
                case 'list':
...
//# sourceMappingURL=industry-activation-system.js.map